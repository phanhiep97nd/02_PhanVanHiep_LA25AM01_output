...
public class CaroLogic {
	 ...
	 * 
	 * @param point
	 *            Tọa độ của điểm vừa được đánh
	 * @return true nếu thắng, false nếu chưa thắng
	 */
	public boolean checkWin(Point point) {
		// kiểm tra checkWinCol hoặc checkWinRow hoặc checkWinAntiDiagonal =
		// true
		return (checkWinCol(point) || checkWinRow(point) || checkWinDiagonal(point));
		// kết thúc hàm
	}
	...
	private boolean checkWinRow(Point point) {
		...
		// Tạo một biến đếm bằng 0
		int count = 0;
		// duyệt ở trên vị trí hiện tại 4 ô và ở dưới vị trí hiện tại 4 ô
		for (int i = -4; i <= 4; i++) {
			// nếu vị trí hiện tại và vị trí kiểm tra có giá trị giống nhau
	 ...
	 * @return true nếu thắng, false nếu chưa thắng
	 */
	private boolean checkWinDiagonal(Point point) {
		// getX trả về double nên cần chuyển về kiểu int
		int x = (int) point.getX();
		// getY trả về double nên cần chuyển về kiểu int
		int y = (int) point.getY();
		...
		// nếu ra khỏi vòng for tức không có giá trị thỏa mãn trả về false
		return false;
		// kết thúc hàm
	}
	 ...
	 *
	 * @param x
	 *            Tọa độ x của ô bàn cờ
	 * @param y
	 *            Tọa độ y của ô bàn cờ
	 ...
	 * 
	 * @param positionModel
	 *            Đối tượng thế cờ là ma trận 5x5
	 * @param subB
	 *            Đối tượng bàn cờ con là ma trận 5x5
	 * @return
	 */
	private Point findPoint(PositionModel positionModel, String[][] subB) {
		// Tạo một biến kiểu Point để lưu tọa độ trả về
		Point point = null;
		// check vị trí bàn cờ với thế cờ. Nếu check = 0 thì không đánh. Nếu
		// check = 1 thì trả về 1 quân cờ
		int check = 1;
		// đưa thế cờ vào mảng
		...
		flag: for (int i = 0; i < rowMat; i++) {
			...
			for (int j = 0; j < colMat; j++) {
				...
				// switch case để so sánh thế cờ từ file với bàn cờ con lấy từ
				// bàn cờ
				switch (position[i][j]) {
